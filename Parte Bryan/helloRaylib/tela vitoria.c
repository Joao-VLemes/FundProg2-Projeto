...
bool win = false; // <-- A tela de vitória usará esta variável global
...
static GAME correct_game; // <-- A tela de vitória usará esta para mostrar o nome
...
void run_gameplay() {
    ...
    while (!WindowShouldClose() && !win) {
        ...
        if (IsKeyPressed(KEY_ENTER) && strcmp(search_results[selected_search_index].name, "") != 0) {
            ...
            else win = true; // <-- GATILHO DA VITÓRIA! O loop vai parar.
    ...
    // ============================================================================
    // --- INÍCIO DA TELA DE VITÓRIA ---
    // Este código só é executado se o loop acima parar por causa de 'win = true'
    // ============================================================================
    if (win && !WindowShouldClose())
    {
        // Loop da Tela de Vitória:
        // Fica "preso" aqui até o usuário pressionar ENTER ou fechar a janela
        while (!WindowShouldClose())
        {
            // Se o usuário pressionar ENTER, quebra o loop da tela de vitória
            ...
            BeginDrawing();
            ClearBackground((Color){30, 30, 30, 255}); // Usa o mesmo fundo do jogo

            // Mensagem de Vitória
            const char *msgVitoria = "VOCE VENCEU!";
            int tamTexto = MeasureText(msgVitoria, 80);
            int posX = (screen_width - tamTexto) / 2;
            int posY = screen_height / 2 - 80;
            DrawText(msgVitoria, posX, posY, 80, GREEN);

            // Mensagem com o nome do jogo (usa a variável global 'correct_game')
            const char *msgJogo = TextFormat("O jogo era: %s", correct_game.name);
            int tamJogo = MeasureText(msgJogo, 30);
            posX = (screen_width - tamJogo) / 2;
            posY = screen_height / 2 + 20;
            DrawText(msgJogo, posX, posY, 30, RAYWHITE);

            // Mensagem para fechar
            const char *msgFechar = "Pressione ENTER para fechar";
            int tamFechar = MeasureText(msgFechar, 20);
            posX = (screen_width - tamFechar) / 2;
            posY = screen_height / 2 + 80;

            // Efeito de piscar para a mensagem
            if ((GetTime() * 2.0f) - (int)(GetTime() * 2.0f) > 0.5f)
            {
                DrawText(msgFechar, posX, posY, 20, LIGHTGRAY);
            }
    ...
    // --- Limpeza / Unload ---
    // O código continua daqui após a tela de vitória
...
int main() {
    ...
    // Como o 'main' não tem um loop, o jogo fechará
    // assim que 'run_gameplay' (incluindo a tela de vitória) terminar.
    
    CloseWindow(); // Adicionado para garantir que a janela feche corretamente
